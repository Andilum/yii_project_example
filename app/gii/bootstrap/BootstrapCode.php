...
class BootstrapCode extends CCodeModel {
    ...
    public $sortfiled;
    public $parent_fild;
    public $baseControllerClass = 'ControllerAdmin';
    private $_modelClass;
    private $_table;
    ...
    public function isColumbDate($column) {
        return (stripos($column->dbType, 'timestamp') !== false || $column->dbType == 'datetime' || $column->dbType == 'date');
    }
    ...
    public function generateActiveControlGroup($modelClass, $column) {
        if ($this->isColumBoll($column))
            ...
            return "\$form->textAreaControlGroup(\$model,'{$column->name}',array('rows'=>6, 'cols'=>50))";
        else {
            if (stripos($column->dbType, 'enum') !== false) {

                return "\$form->dropDownListControlGroup(\$model,'{$column->name}'," . $this->getArrauenum($column->dbType) . ")";
            } else {
                if ($this->isColumbDate($column)) {
                    $this->isdate=true;
                       return "\$form->textFieldControlGroup(\$model,'{$column->name}',array('class'=>'datevibor'))";  //$this->getWidgetDate($column);
                } else {
