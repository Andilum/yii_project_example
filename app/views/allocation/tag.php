...
<div class="hashtags">
    ...
    <?=Yii::t('search', 'Хэштеги')?>  <a href="<?= Yii::app()->createUrl('/allocation/view', array('id' => $allocation->id)) ?>"><?= $allocation->name ?> <?= $allocation->alloccat->name ?></a>
    </div>
    <div class="hashtags-search">
        ...
        <?= CHtml::ajaxButton(Yii::t('search', 'Найти'), Yii::app()->createUrl('/allocation/tag', array('id' => $allocation->id)), array(
    ...
    <div class="hashtags-table">
        <div class="hashtags-table-row hashtags-table-row_ttl">
            ...
            <?php foreach ($dataProvider->sort->attributes as $sortParam => $attr): ?>
                <div class="hashtags-table-cell">
                    <?php $url = Yii::app()->createUrl('/allocation/tag', array('id' => $allocation->id));
                    $direction = '';
                    $sortClass = '';
                    if ($sort[0] == $sortParam) {
    ...
    <?php if ($dataProvider->totalItemCount > Tag::POST_LIMIT_ON_FIRST_LOAD): ?>
        ...
        <div class="hashtags-more">
            ...
            <?=Yii::t('search', 'Показать')?>   <?= CHtml::ajaxLink(Yii::t('search','следующие 20 хэштегов'), Yii::app()->createUrl('/allocation/tag', array('id' => $allocation->id)), array(
